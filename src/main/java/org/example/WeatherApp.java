...
//        //css query language
        try {
            ...
            String temperature = page.select(".now-weather .unit_temperature_c").text();
            System.out.println("Температура: " + temperature + "°C");

            // Извлечение температуры по ощущению
            String feelTemperature = page.select(".now-feel .unit_temperature_c").text();
            System.out.println("Температура по ощущению: " + feelTemperature + "°C");

            // Извлечение информации о ветре
            String wind = page.select(".now-info-item.wind .unit_wind_m_s").text();
            System.out.println("Ветер: " + wind);

            // Извлечение информации о давлении
            String pressure = page.select(".now-info-item.pressure .unit_pressure_mm_hg").text();
